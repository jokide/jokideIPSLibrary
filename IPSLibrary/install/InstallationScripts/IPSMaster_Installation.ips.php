<?
	...
	/**@defgroup IPSMaster_visualization IPSMaster Visualisierung
	 * @ingroup IPSMaster
	 * @{
	 *
	 * Visualisierungen von IPSMaster
	 *
	 * IPSMaster WebFront Visualisierung:
	 *
	 *
	 *@}*/

	/**@defgroup IPSMaster_install IPSMaster Installation
	 * @ingroup IPSMaster
	 * @{
	 *
	 * Script zur kompletten Installation der IPSMaster Steuerung.
	 *
	 * Vor der Installation muß das File IPSMaster_Configuration.inc.php an die persönlichen
	 * Bedürfnisse angepasst werden.
	 *
	 * @page rquirements_IPSMaster Installations Voraussetzungen
	 * - IPS Kernel >= 2.50
	 ...
	 *
	 * @page install_IPSMaster Installations Schritte
	 * Folgende Schritte sind zur Installation der IPSMaster Ansteuerung nötig:
	 * - Laden des Modules (siehe IPSModuleManager)
	 * - Konfiguration (Details siehe Konfiguration)
	 * - Installation (siehe IPSModuleManager)
	 *
	 * @file          IPSMaster_Installation.ips.php
	...
	if (!isset($moduleManager)) {
		...
		$moduleManager = new IPSModuleManager('IPSMaster');
	}
	...
	IPSUtils_Include ("IPSMaster.inc.php",                "IPSLibrary::app::modules::IPSMaster");
	...
	// Add Scripts
	$scriptIdActionScript  = IPS_GetScriptIDByName('IPSMaster_ActionScript', $CategoryIdApp);

	// Profiles
	CreateProfile_Switch ("IPSMaster_Light", 'Aus', 'An', $Icon="", -1, 0x00ff00, $IconOff="BulbOff", $IconOn="BulbOn");
	...
	foreach ($lightConfig as $deviceName=>$deviceData) {
		$deviceType = $deviceData[IPSMaster_TYPE];
		...
		switch ($deviceType) {
			...
			case IPSMaster_TYPE_DIMMER:
				$switchId = CreateVariable($deviceName,                       0 /*Boolean*/, $categoryIdSwitches,  $idx, '~Switch',        $scriptIdActionScript, false, 'Bulb');
				$levelId  = CreateVariable($deviceName.IPSMaster_DEVICE_LEVEL, 1 /*Integer*/, $categoryIdSwitches,  $idx, '~Intensity.100', $scriptIdActionScript, false, 'Intensity');
				break;
			case IPSMaster_TYPE_RGB:
				$switchId = CreateVariable($deviceName,                       0 /*Boolean*/, $categoryIdSwitches,  $idx, '~Switch',        $scriptIdActionScript, false, 'Bulb');
				$colorId  = CreateVariable($deviceName.IPSMaster_DEVICE_COLOR, 1 /*Integer*/, $categoryIdSwitches,  $idx, '~HexColor',      $scriptIdActionScript, false, 'HollowDoubleArrowRight');
				$levelId  = CreateVariable($deviceName.IPSMaster_DEVICE_LEVEL, 1 /*Integer*/, $categoryIdSwitches,  $idx, '~Intensity.100', $scriptIdActionScript, false, 'Intensity');
				break;
			default:
	...
	foreach ($programConfig as $programName=>$programData) {
		...
		CreateProfile_Associations ('IPSMaster_'.$programName, $programAssociations, "ArrowRight");
		$programId = CreateVariable($programName, 1 /*Integer*/, $categoryIdPrograms,  $idx,  'IPSMaster_'.$programName, $scriptIdActionScript, 0);
		$idx = $idx + 1;
	}
	...
	$lightConfig = IPSMaster_GetLightConfiguration();
	foreach ($lightConfig as $deviceName=>$deviceData) {
		$component = $deviceData[IPSMaster_COMPONENT];
		...
		if ($componentClass=='IPSComponentSwitch_Homematic') {
			...
			if ($variableId===false) {
				...
				$messageHandler->RegisterOnChangeEvent($variableId, $component, 'IPSModuleSwitch_IPSMaster,');
			}
		// EIB
	...
	if ($WFC10_Enabled) {
		...
		$webFrontConfig = IPSMaster_GetWebFrontConfiguration();
		$order = 10;
			...
			foreach($tabData as $WFCItem) {
				...
				switch($WFCItem[0]) {
					...
					case IPSMaster_WFCCATEGORY:
						...
						break;
					case IPSMaster_WFCGROUP:
					case IPSMaster_WFCLINKS:
						$categoryId = IPS_GetCategoryIDByName($WFCItem[2], $tabCategoryId);
						if ($WFCItem[0]==IPSMaster_WFCGROUP) {
	...
	if ($mobile_Enabled ) {
		...
		$order = 10;
		foreach (IPSMaster_GetMobileConfiguration() as $roomName=>$roomData) {
			if (is_array($roomData)) {
				...
				foreach($roomData as $roomItem) {
					...
					switch($roomItem[0]) {
						...
						case IPSMaster_WFCLINKS:
							$instanceId = $roomId;
							if ($roomItem[0]==IPSMaster_WFCGROUP) {
